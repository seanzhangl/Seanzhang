...
globals [transactionPrice ;; last transaction price
         ...
         bestBidID        ;; 【buyer】 with current best bid 不是ID，而是agent，切记
         bid?             ;; bid available?
         bestAsk          ;; current best Ask
         bestAskID        ;; seller with current best ask
         ask?             ;; ask available?  这个很容易和原语混淆，还是用别的词好
         predictedq       ;; predicted market q 预测的市场成交量
         predictedp       ;; predicted market p 预测的市场成交价
        ]
...
to setup
  ...
  ;; populate the patches with the number of each agents randomly
    ask n-of numberOfBuyers patches [ 
      sprout-buyers 1 [init-buyer]   ;生成一个买方并初始化
      ]
    ask n-of numberOfSellers patches [
      sprout-sellers 1 [init-seller]   ;生成一个卖方并初始化
      ]; 使用瓦片生成agent，为什么这么设定？？？？
  ;; plot the demand-supply curves 绘制供给需求曲线
    ;;create-demand-supply  先计算供给与需求
  ;; calculate the max surplus in this market 计算市场最大盈余
    calc-max-surplus ;直接调用了计算市场最大盈余的历程
  ;; initialize actual surplus
    set actualSurplus 0.0 
  ;; initialize the orderbook 订单簿初始化
    init-orderBook
  ;; create traders agentset   买卖双方都是trader集合里的
    set traders turtles with [breed = buyers or breed = sellers]
  ;; reset the tickcount
    reset-ticks
  ;; draw supply and demand curve  绘制供给需求曲线
  create-demand-supply  
  end

;; restore the default parameters   恢复面板各控制量的初始值
to restore-defaults
...
;; initialize buyer
to init-buyer
    set color black   ;; set so does not show in world view    不现示世界不就的了
    set value random-float maxBuyerValue  ;不超过设定最高值随机取值
    set traded? false  ;没有交易过
  end
  
;; initialize seller
to init-seller
    set color black  ;; set so does not show in world view
    set cost random-float maxSellerCost  ;不超过设定最高值随机取值
    set traded? false   
  end
    ...
    [
      ifelse breed = buyers
        ...
        [ ;; get ask price
          ...
          if (askPrice < cost) and (constrained) [ show "Seller ask below cost with ZI-C"]
...
;; calculate the maximum possible surplus from market  计算市场最大可能盈余
;; and calculate predicted equilibrium  并计算预测均衡
to calc-max-surplus
  let bids 0
  let asks 0
  let surpluses 0
  
  set surpluses 0.0   ;这句和上一句有什么区别？？？？
  ;; grab the sorted bids  抓取排序的买方投标
  set bids sort-by [ ?1 > ?2 ] [value] of buyers ;这句相当于直接reverse sort
  ;; grab the asks  抓取排序的卖方要价
  set asks sort-by [ ?1 < ?2 ] [cost] of sellers  ;这句相当于直接 sort
  ;; we now have a list of bids sorted from highest to lowest 买方报价从高到低
  ;; and a list of asks sorted from lowest to highest  卖方报价从低到高
  ;; now shorten either bids or asks based on which has fewest  将多出来的报价消掉
    if (numberOfBuyers != numberOfSellers) [     ;直接比较两个列表的长度不就行了？
      ifelse (numberOfBuyers < numberOfSellers)
        [ set asks sublist asks 0 (numberOfBuyers) ]    ;截短
        [ set bids sublist bids 0 (numberOfSellers) ]
      ]
  ;; sum the positive surpluses for the sorted               成对排序的买卖双方报价列表的盈余总和 
  ;; pairwise bids and asks from highest to lowest
    set predictedq 0    ;成交量清零
    set predictedp 200   ;成交价设成200，哪来的数？？
    (foreach bids asks [
      if ( ?1 - ?2 > 0.0) [   ;如果列表中配对的买方报价高于卖方报价
      set predictedq predictedq + 1   ;预测成交量+1，明白了，这里交易的都是一件，没有数量！！！！
      set predictedp (?1 + ?2) / 2   ;预测成交价取两者平均值
      set surpluses (surpluses + ?1 - ?2) ]   ;盈余是两者价格差值
      ]
     );小括号有毛用？
   ;; now sum the positive surpluses to get maxSurplus
     set maxSurplus surpluses   ;把surplus的值传递给全局变量
end
